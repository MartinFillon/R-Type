...
namespace ecs {
    ...
    class ComponentFactory {
      public:
        /// @brief `enum` representing the mode of the component factory, server's one is much more complete than
        /// client's one.
        enum class Mode { Server, Client };

        /// @brief Generates a `ComponentFactory` creating in the process an unorderd map of function and store it,
        /// string as a Key and function as a value.
        /// @param registry a `std::shared_ptr<Registry> &` representing the reference of the registry of the game.
        /// @param mode `Mode::*` representing the mode of the component factory, actions will be different server and
        /// client versions.
        ...
        /// @param file a `const std::string&` representing the reference to the json config file of the newly created
        /// entity.
        /// @return `Entity` representing the newly created entity.
        Entity createEntity(const std::string &file);

        /// @brief Creates a component by linking the factory's functions map and the given `component` string into the
        /// given entity `e` from the given json node reference `node`.
        /// @param e a `const Entity` representing the entity to which to add the new component.
        /// @param component a `const std::string &` representing the reference of the name of the component to add to
        /// the entity.
        /// @param node a `const nlohmann:json &` representing the reference of the json node from which the new
      ...
      private:
        ...
        /// @brief The map of the factory's functions indexed by an `std::string` key is the map of name component
        /// (string) as key and function (create[name]Component) as value to create the component
